...
public class UserIdGenerator {
	...
	public String generateUserId(String emailId) {
		...
		// Take the first 5 characters of emailId, or pad with zeroes if less than 5
		// characters
		...
		// Alternate between adding a character from emailPrefix and a digit from
		// timestampDigits
		...
		if (timestampIndex < timestampDigits.length) {
			userIdBuilder.append(
					String.join("", Arrays.copyOfRange(timestampDigits, timestampIndex, timestampDigits.length)));
		}
